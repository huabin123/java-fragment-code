...
- 就是在程序运行时，把热点代码（经常执行的代码）编译成机器码
- 就像翻译：第一次慢慢翻译，翻译好了直接用，不用每次都翻译

**为什么需要JIT？**
```
解释执行（慢）：
每次执行 → 翻译字节码 → 执行
每次执行 → 翻译字节码 → 执行  （重复翻译，慢！）

JIT编译（快）：
第一次执行 → 翻译字节码 → 执行
第二次执行 → 直接执行机器码（不用翻译，快！）
```
...
**一句话记住：**
> JIT就是把热点代码编译成机器码，C1快但粗糙，C2慢但精细，分层编译两者兼顾！
