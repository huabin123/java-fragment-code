...
**JVM命令行工具是什么？**
- 就是JDK自带的一堆诊断工具，用来查看JVM运行状态
- 就像医生的听诊器、体温计，用来给JVM"体检"

**核心工具速查表：**

| 工具 | 作用 | 一句话说明 |
|------|------|-----------|
| **jps** | 查看Java进程 | 找到Java进程的PID |
| **jstat** | 查看GC统计 | 看GC频率、时间、内存使用 |
| **jinfo** | 查看/修改JVM参数 | 查看运行时配置 |
| **jmap** | 导出堆dump | 内存快照，分析内存问题 |
| **jstack** | 导出线程dump | 线程快照，分析死锁、CPU高 |
| **jcmd** | 万能工具 | 集成了上面所有功能 |
...
**注意事项：**
- ⚠️ **jmap -dump会触发Full GC**，生产环境慎用
- ⚠️ **dump文件很大**，确保磁盘空间足够
- ⚠️ **jstack可能导致应用暂停**，快速执行
- ✅ **优先使用jcmd**，功能更全，影响更小

**一句话记住：**
> jps找进程，jstat看GC，jmap导内存，jstack导线程，jcmd是万能！
