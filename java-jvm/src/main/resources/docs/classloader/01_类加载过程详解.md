...
**类加载是什么？**
- 就是把`.class`文件（字节码）加载到JVM内存里，让它变成可以用的Java类
- 就像把一本书从书架上拿下来，翻开，准备阅读

**类加载的5个步骤（记住口诀：加验准解初）**
1. **加载**：把.class文件读到内存里
2. **验证**：检查这个文件是不是合法的（防止有人搞破坏）
3. **准备**：给类的静态变量分配内存，设置默认值（如int默认0）
4. **解析**：把符号引用转成直接引用（把"地址"变成真正的内存地址）
5. **初始化**：执行静态代码块，给静态变量赋真正的值
...
// 1. 加载：读取User.class文件
// 2. 验证：检查文件格式是否正确
// 3. 准备：给count分配内存，设置默认值0
// 4. 解析：处理各种引用
// 5. 初始化：执行静态代码块，count变成100，打印"类初始化"
```

**什么时候会触发类加载？**
- ✅ 第一次创建对象时：`new User()`
- ✅ 访问静态变量或方法时：`User.count`
- ✅ 反射调用时：`Class.forName("User")`
- ✅ 初始化子类时，会先初始化父类
- ❌ 只是定义变量不会触发：`User user;`（没有new）

**重点注意：**
- 每个类只会被加载一次（单例模式的基础）
- 准备阶段只设置默认值，初始化阶段才赋真正的值
- 静态代码块只执行一次，在类初始化时

**一句话记住：**
> 类加载就是把.class文件变成内存中可用的类，分5步走，只做一次！
