...
2. 发现问题
   → 通过监控找到瓶颈（CPU高？内存泄漏？GC频繁？）
   
3. 分析原因
   → 深入分析根本原因（为什么CPU高？哪里在循环？）
...
// 4. 避免大对象
byte[] small = new byte[1024];  // 好
byte[] large = new byte[10 * 1024 * 1024];  // 不好
```
...
**一句话记住：**
> 性能调优要数据驱动，先监控找瓶颈，再优化验证效果，不要过早优化！
